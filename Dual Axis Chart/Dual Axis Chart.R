...
portfolioPrices <- portfolioPrices[apply(
  portfolioPrices,
  1,
...
portfolioReturns <- ROC(
  portfolioPrices,
  type = "discrete"
)

autoplot(portfolioReturns,facets = NULL) +
  scale_color_manual(values = c(WFG = "black", GSPC = "red")) +
...
ggplot(data = Prices, aes(Index, WFG)) +
  ...
  scale_color_manual(values = c(WFG = "red", GSPC = "black")) +
  scale_y_continuous(sec.axis = sec_axis(~ . / fac, name = "S&P 500 Closes")) +
  labs(x = "Date", y = "WFG Close Price", color = "") +
  ggtitle("Stock Price") +
  theme_bw()
